...
def _call_responses(api_key: str, model: str, instructions: str,
                    input_items: list[dict], tools: list[dict]) -> dict:
    payload = {
        "model": model,
...
def run_query(question: str, cache_read: bool | None = None,
    ...
    instructions = _runtime_preamble(read, write) + PROCEDURE_PATH.read_text(encoding="utf-8")
    ...
    for _ in range(MAX_TOOL_TURNS):
        ...
        if not calls:
            # Model answered in prose without submit_answer — accept as fallback.
            final = {"answer": _message_text(output_items), "entities_resolved": [],
                     "sources_cited": [], "status": "answered", "time_sensitive": False,
                     "saf": False, "reused_query_id": None}
            break
        for call in calls:
            try:
