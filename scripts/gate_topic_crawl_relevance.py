...
def main() -> int:
    ...
    for topic_id in topic_ids:
        ...
        fm = frontmatter(ROOT / "entities" / "topic" / f"{topic_id}.md")
        topic = {"topicId": topic_id, "displayName": fm["displayName"], "definition": "", "crawlPrompt": ""}
        note = (ROOT / "entities" / "topic" / f"{topic_id}.md").read_text(encoding="utf-8")
        topic["definition"] = note.split("## Definition", 1)[1].split("##", 1)[0].strip()
        topic["crawlPrompt"] = note.split("## Crawl Prompt", 1)[1].split("##", 1)[0].replace("```text", "").replace("```", "").strip()
        seen, candidates = set(), []
        for name in names:
