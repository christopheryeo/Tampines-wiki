...
DOMESTIC = re.compile(
    r"\b(singapore|s'pore|mindef|saf|parliament|mha|mddi|rsaf|rsn|"
    r"national service|singapore strait|malacca strait)\b",
    re.IGNORECASE,
)
FORCED_RESPONSE = re.compile(
    r"\b(minister|government|parliament|ministry|mha|mindef|police|agency|"
    r"prime minister|foreign minister)\b",
    re.IGNORECASE,
)
FAULT_LINE = re.compile(
    r"\b(race|racial|religion|national service|sovereignty|foreign influence|"
    r"foreign interference|falsehood|disinformation|social cohesion|"
    r"procurement|neutrality|multicultural|cyber attack|critical infrastructure)\b",
    re.IGNORECASE,
)
...
def primary_review(metrics: dict) -> dict:
    if not metrics["domesticEvidence"]:
        return {
            "disposition": "dismiss",
            "reason": "No Singapore-linked evidence in the compiled article sample.",
        }
    if (
