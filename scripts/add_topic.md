...
6. **Build the complete proposed record.** Produce one in-memory registration object containing
   ...
   Prompt. Keep the definition concise. Keep keyword seeds narrow enough for canonical assignment;
...
8. **Write both source records as one transaction.** Insert exactly one mapping in
   ...
   Template with the approved definition, one Crawl Prompt, an empty Coverage section, and no
   invented Notes. Do not create either record unless both can be completed.
