...
| `articleDateRange.timezone` | IANA timezone string | Optional | Timezone used to interpret both date boundaries. Defaults to `Asia/Singapore`. | `Asia/Singapore` |
...
identifies the remainder not yet crawled on the current Singapore calendar date, sorts them by
...
it is `Asia/Singapore`. This value tells the endpoint how the calendar-day boundaries should be
interpreted. Examples of syntactically recognizable IANA values include `Asia/Singapore` and
`UTC`; values such as `SGT`, `Singapore time`, or `+08:00` are not substitutes for this parameter.

The timezone applies to the article publication window. The automatic definition of “not crawled
today” remains anchored to the Singapore calendar date so that batch selection is consistent across
callers and systems.
...
> `2026-08-01` to `2026-08-10` in `Asia/Singapore`.
...
Expected interpretation: use the default `Asia/Singapore` timezone; select the first five eligible
...
```json
{
  "topic": "Example Emerging Defence Technology",
  "numberOfTopics": 1,
  "articleDateRange": {
    "from": "2026-08-01",
    "to": "2026-08-10",
...
4. `articleDateRange.timezone` — optional IANA timezone; default `Asia/Singapore`.
...
- When only `numberOfTopics` is supplied, eligible topics are those whose `lastCrawledAt` is blank or
  resolves to a Singapore date earlier than today's Singapore date. A nonblank timestamp from an
...
```json
{
  ...
  "topic": {
    ...
    "crawlPrompt": "Find news articles about military aviation, air-force capabilities, combat aircraft, air-defence systems, aerial refuelling, transport aircraft and related acquisitions or exercises."
  },
  "articleDateRange": {
    "from": "YYYY-MM-DD",
    "to": "YYYY-MM-DD",
...
6. **Select a numbered batch.** If only `numberOfTopics` is supplied, freshly read every active
   ...
   remaining checkpoint with today's date in `Asia/Singapore`, sort eligible topics by `displayName`
