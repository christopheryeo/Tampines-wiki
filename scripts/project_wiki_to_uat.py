...
``prepare``
    Reads the wiki and UAT, assembles all 22,820 canonical projections, assigns
    stable IDs to the 816 wiki-only records, and writes a hashed bundle. No wiki
    or database write occurs.

``verify-bundle``
    Recomputes bundle hashes and validates identities, parent schemas, child
    multisets, the 816/0/0 initial delta contract, and rollback artifacts.
...
``load``
    ...
    816/0/0 diff, and executes the rollback-on-failure UAT transaction. The
    target is fixed to ``MSM_dataset_UAT``; production is prohibited.
"""
...
ARTIFACT_ROOT = ROOT / "runs" / "2026-07-29" / "artifacts" / "wiki-uat-reconciliation"
RECONCILIATION = ARTIFACT_ROOT / "reconciliation-baseline.json"
IDENTITY_BASELINE = ARTIFACT_ROOT / "uat-identity-baseline.tsv"
DIRECT_REVIEW = ARTIFACT_ROOT / "direct-ai-review-50.json"
JULY29_REVIEW = ROOT / "runs" / "2026-07-29" / "artifacts" / "july-2026-ai-reviewed-enrichment.json"
JULY23_ARTICLES = ROOT / "runs" / "2026-07-23" / "artifacts" / "enriched-radar-uat-bundle" / "articles.tsv"
...
EXPECTED_WIKI_COUNT = 22_820
EXPECTED_INSERTS = 816
EXPECTED_EXISTING = 22_004
EXPECTED_LEGACY = 5_192
EXPECTED_SHARED = 16_812
EXPECTED_DIRECT_REVIEW = 50
...
def load_reviews() -> dict[str, dict[str, Any]]:
    ...
    with JULY23_ARTICLES.open(encoding="utf-8", newline="") as handle:
        for row in csv.reader(handle, delimiter="\t"):
            ...
            reviews[source_id] = _review_record(
                ...
                "runs/2026-07-23/artifacts/enriched-radar-uat-bundle/articles.tsv",
            )
    ...
    for assessment in july29.get("assessments", []):
        ...
        reviews[source_id] = _review_record(
            ...
            values.get("outletName") or "",
            values.get("outletCountry") or "",
            "runs/2026-07-29/artifacts/july-2026-ai-reviewed-enrichment.json",
        )
    ...
    for group in direct.get("groups", []):
        for source_id in group["sourceIds"]:
            ...
            reviews[source_id] = _review_record(
                ...
                "runs/2026-07-29/artifacts/wiki-uat-reconciliation/direct-ai-review-50.json",
            )
...
def verify_bundle_dir(bundle_dir: Path) -> dict[str, Any]:
    ...
    if manifest.get("targetDatabase") != TARGET_DATABASE:
        raise ProjectionError("bundle target is not MSM_dataset_UAT")
    ...
    if len(records) != EXPECTED_WIKI_COUNT:
        raise ProjectionError("projection bundle does not contain 22,820 records")
    source_ids = set()
    article_ids = set()
    backfills = 0
    ...
    if len(identity_lines) != EXPECTED_WIKI_COUNT + 1:
        raise ProjectionError("identity-map.tsv does not contain 22,820 data rows")
    mapped = {}
    ...
    if "MSM_dataset`" in load_sql.replace(f"`{TARGET_DATABASE}`", ""):
...
def verify_current_bundle_dir(
    ...
    if manifest.get("targetDatabase") != TARGET_DATABASE:
        raise ProjectionError("current bundle target is not MSM_dataset_UAT")
    counts = manifest.get("counts", {})
    records = load_projection_records(bundle_dir)
...
def render_chunked_insert_sql(
    inserts: list[dict[str, Any]],
    bundle_id: str,
    start_count: int,
    final_count: int,
) -> str:
    """Render an insert-only transaction without one oversized procedure packet."""

    guard = "tmp_uat_guard_" + re.sub(r"[^a-z0-9]", "", bundle_id.lower())[:12]
    parent_rows = [[p["article"].get(column) for column in ARTICLE_COLUMNS] for p in inserts]
    coverage_columns = [
        "article_id", "coverage_id", "coverage_type", "display_name",
        "country", "media_outlet_category", "url",
    ...
    coverage_rows = [
        [p["article"]["article_id"], *[row.get(column) for column in coverage_columns[1:]]]
        for p in inserts for row in p["coverage"]
    ]
    media_columns = ["article_id", "media_id", "file_name", "media_url", "media_type", "source"]
    media_rows = [
        [p["article"]["article_id"], *[row.get(column) for column in media_columns[1:]]]
        for p in inserts for row in p["media"]
    ]
    tag_rows = [[p["article"]["article_id"], tag] for p in inserts for tag in p["tags"]]
    group_rows = [[p["article"]["article_id"], group] for p in inserts for group in p["userGroups"]]

    def single_row_inserts(table: str, columns: list[str], rows: list[list[Any]]) -> str:
        prefix = f"INSERT INTO `{table}` ({','.join(f'`{column}`' for column in columns)}) VALUES "
        return "\n".join(
            prefix + "(" + ",".join(_sql_literal(value) for value in row) + ");"
            for row in rows
        )

    body = "\n".join(filter(None, [
        single_row_inserts("UAT_articles", ARTICLE_COLUMNS, parent_rows),
        single_row_inserts("UAT_article_coverage", coverage_columns, coverage_rows),
        single_row_inserts("UAT_article_media", media_columns, media_rows),
        single_row_inserts("UAT_article_tags", ["article_id", "tag"], tag_rows),
        single_row_inserts("UAT_article_user_groups", ["article_id", "user_group_id"], group_rows),
    ]))
    orphan_free = """NOT EXISTS (
      SELECT 1 FROM `UAT_article_coverage` c LEFT JOIN `UAT_articles` a ON a.article_id=c.article_id WHERE a.article_id IS NULL
    ) AND NOT EXISTS (
      SELECT 1 FROM `UAT_article_media` m LEFT JOIN `UAT_articles` a ON a.article_id=m.article_id WHERE a.article_id IS NULL
    ) AND NOT EXISTS (
      SELECT 1 FROM `UAT_article_tags` t LEFT JOIN `UAT_articles` a ON a.article_id=t.article_id WHERE a.article_id IS NULL
    ) AND NOT EXISTS (
      SELECT 1 FROM `UAT_article_user_groups` g LEFT JOIN `UAT_articles` a ON a.article_id=g.article_id WHERE a.article_id IS NULL
    )"""
    return f"""USE `{TARGET_DATABASE}`;
DROP TEMPORARY TABLE IF EXISTS `{guard}`;
CREATE TEMPORARY TABLE `{guard}` (`ok` TINYINT NOT NULL CHECK (`ok` = 1));
START TRANSACTION;
INSERT INTO `{guard}` VALUES ((SELECT COUNT(*) = {start_count} FROM `UAT_articles`));
DELETE FROM `{guard}`;
{body}
INSERT INTO `{guard}` VALUES ((SELECT COUNT(*) = {final_count} FROM `UAT_articles`));
DELETE FROM `{guard}`;
INSERT INTO `{guard}` VALUES ((SELECT {orphan_free}));
COMMIT;
DROP TEMPORARY TABLE `{guard}`;
"""
...
def load(args: argparse.Namespace) -> int:
    ...
    if len(sql.encode("utf-8")) > 8 * 1024 * 1024:
        if int(manifest["delta"]["updates"]) or int(manifest["delta"]["deletes"]):
            raise ProjectionError("oversized chunked load currently permits insert-only bundles")
        records = load_projection_records(bundle_dir)
        insert_ids = {int(value) for value in manifest["delta"]["insertArticleIds"]}
        inserts = [
            row["projection"] for row in records
            if int(row["projection"]["article"]["article_id"]) in insert_ids
        ]
        if len(inserts) != len(insert_ids):
            raise ProjectionError("chunked load insert identities differ from verified bundle")
        sql = render_chunked_insert_sql(
            inserts,
            manifest["bundleId"],
            int(manifest["counts"]["existingUatArticles"]),
            int(manifest["counts"]["wikiArticles"]),
        )
...
def verify_rollback(args: argparse.Namespace) -> int:
    ...
    fk_output = run_mysql(
        ...
        "WHERE CONSTRAINT_SCHEMA='MSM_dataset_UAT' "
