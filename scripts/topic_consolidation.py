...
def load_taxonomy(path: Path = TAXONOMY_PATH) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    if not 60 <= len(topics) <= maximum <= ABSOLUTE_MAXIMUM_ACTIVE_TOPICS:
        raise ConsolidationError(
            f"canonical taxonomy count {len(topics)} is outside 60–{maximum} or maximum exceeds "
            f"{ABSOLUTE_MAXIMUM_ACTIVE_TOPICS}"
        )
    for row in topics:
        allowed_fields = {"topicId", "displayName", "category", "definition", "keywords"}
        unexpected = set(row) - allowed_fields
        if unexpected:
            raise ConsolidationError(
                f"canonical topic {row.get('topicId')} has unexpected fields: "
                f"{', '.join(sorted(str(key) for key in unexpected))}; "
                "check quoting of commas in YAML flow values"
            )
...
def pattern_score(topic: dict[str, Any], sources: list[tuple[str, float]]) -> tuple[float, list[str]]:
    ...
    for value, weight in sources:
        if not value:
            continue
        # One combined search per source is materially faster over the 22,820-article corpus
...
def classify(fragments: dict[str, Any], topics: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for topic in topics:
        if topic["topicId"] == "general-defence-security":
            continue
    ...
    fallback = not scores
    if fallback:
        selected = ["general-defence-security"]
        evidence_by_id = {"general-defence-security": [fragments["category"] or "no narrower evidence"]}
    else:
