...
class WikiUatReconciliationTests(unittest.TestCase):
    ...
    def test_cascade_assigns_only_registered_canonical_topics(self):
        selected = CASCADE.canonical_topic_selection(
            "Ukraine needs air defence against Russian missiles",
            "Russia launched ballistic missiles and Ukraine requested Patriot air defence systems.",
            {"topic": "Ballistic Missile", "category": "National Security"},
            ["Ballistic Missile", "Missile Production"],
        )
        canonical_ids = {
        ...
        self.assertIn("missiles-air-defence", {topic_id for topic_id, _ in selected})
    ...
    def test_read_only_mysql_rejects_mutation_and_production(self):
        ...
        with self.assertRaises(PROJECTION.ProjectionError):
            PROJECTION.run_mysql("SELECT 1 FROM MSM_dataset.articles")
        with self.assertRaises(PROJECTION.ProjectionError):
    ...
    def test_review_artifacts_cover_exact_frozen_wiki_only_population(self):
        reviews = PROJECTOR.load_reviews()
        baseline = json.loads(PROJECTOR.RECONCILIATION.read_text(encoding="utf-8"))
        wiki_only = {row["sourceId"] for row in baseline["wikiOnly"]}
        self.assertEqual(len(wiki_only), 816)
        self.assertTrue(wiki_only <= set(reviews))
        direct = json.loads(PROJECTOR.DIRECT_REVIEW.read_text(encoding="utf-8"))
        direct_ids = [
            source_id
            for group in direct["groups"]
            for source_id in group["sourceIds"]
        ]
        self.assertEqual(len(direct_ids), 50)
        self.assertEqual(len(set(direct_ids)), 50)
    ...
    def test_load_sql_is_uat_only_transaction_with_rollback_handler(self):
        ...
        self.assertIn("USE `MSM_dataset_UAT`;", sql)
        self.assertIn("START TRANSACTION;", sql)
        self.assertIn("ROLLBACK;", sql)
        self.assertIn("RESIGNAL;", sql)
        self.assertIn("COMMIT;", sql)
        self.assertNotIn("USE `MSM_dataset`;", sql)

    def test_chunked_insert_sql_is_guarded_without_procedure_packet(self):
        projection = self.projection()
        for column in PROJECTION.ARTICLE_COLUMNS:
            projection["article"].setdefault(column, None)
        sql = PROJECTOR.render_chunked_insert_sql([projection], "abc123", 10, 11)
        self.assertIn("START TRANSACTION;", sql)
        self.assertIn("CHECK (`ok` = 1)", sql)
        self.assertIn("SELECT COUNT(*) = 10", sql)
        self.assertIn("SELECT COUNT(*) = 11", sql)
        self.assertIn("COMMIT;", sql)
        self.assertNotIn("CREATE PROCEDURE", sql)
        self.assertNotIn("USE `MSM_dataset`;", sql)
