# Tampines Wiki — Project Home

**Status:** Confirmed project (formalized 2026-09-04 SGT)  
**Dropbox home:** `/Users/chrisyeo/Dropbox/Work/CEO (Sentient)/Customer Folders/tampines-wiki`  
**Git:** https://github.com/christopheryeo/Tampines-wiki  
**Build log:** `../DEVELOPMENT.md`  
**Process map:** `../docs/development-process.md`  
**Journal (Drive only):** https://docs.google.com/document/d/1LsucY9zwEWXukVwGftmpzUhfuguSj1VRxs_zK7EU6lw/edit  
**ClickUp:** Tampines Wiki List (Owen)  
**Room:** Tampines

## Delivery next (human)

1. Arivumathi topic-crawling check
2. One more crawl
3. Present final results to `nsiewpin@dsta.gov.sg`

Payment is in. Open risk: overdue Q2 stories (SMTC-691 / 752–755).

## Ownership

| Area | Owner |
|------|--------|
| ClickUp / human chase | Owen |
| DEVELOPMENT.md / Features | Felix |
| PR merge | Giselle |
| Folder map | Leni |
| Inbox / calendar | Vivien |
| Dispatch | Morgan |
